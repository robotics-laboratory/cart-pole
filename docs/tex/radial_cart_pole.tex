...
\begin{tikzpicture}[tdplot_main_coords, scale = 10]
    ...
    \draw[->] (0,0,0) -- (0,0,0.7) node[anchor=south]{\(z\)};
    ...
    % formula
    \draw (0.7,0,0.6) node[fill=yellow!30, draw, rounded corners] {\Large \(\ddot{\theta} = - \frac{\ddot{\phi} r \cos \theta + g\sin \theta}{l_p + \frac{I_p}{m_p l_p}}\)};
\end{tikzpicture}
\end{document}