...
def acc_change_duration(a0, l0, moves_num):
    ...
    for i in range(moves_num):
        ...
        abs_change_timestamp = sign_change_timestamp + timestamp
...
def collect_data(config, cart_pole, moves_num):
    a0 = a = config.max_acceleration / ((moves_num + 1) * 2)
    l0 = config.max_position * 0.5
    ...
    while abs_change_timestamps and sign_change_timestamps:
        ...
        if time > sign_change_timestamps[0]:
            a = -a
