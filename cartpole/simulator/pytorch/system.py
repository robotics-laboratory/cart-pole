...
class CartPoleMultiSystem:
    ...
    def advance(self, batch_state: MultiSystemState, inputs: FloatTensor) -> None:
        """
        ...
        """
        # Current state
        cur_st = batch_state.clone()
        steps: int = self.config.discretization.integration_step_n
        # dynamics delta time
        d_time: float = 1 / (steps * self.config.discretization.simulation_step_n)

        # Gravitational constant
        grav: float = self.config.parameters.gravity
        pole_len: float = self.config.parameters.pole_length

        def compute_derivative(state: FloatTensor):
