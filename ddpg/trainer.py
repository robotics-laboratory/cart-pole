...
def compute_reward(state: State):
    angle = abs((state.pole_angle + np.pi) % (2*np.pi) - np.pi)
    return Scalar(value=angle)
...
class Trainer:
    ...
    def select_action(self, state: State, sigma: float, train: bool):
        with torch.no_grad():
            self.actor.eval()
            act = self.actor(state_to_tensor(state).reshape(1, -1))
            self.actor.train()

            self.critic.eval()
            rew1, rew2 = self.target_critic(state_to_tensor(state).reshape((1, -1)), act)
            self.logger.publish('/cartpole/est_reward1', Scalar(value=rew1.numpy()), state.stamp)
            self.logger.publish('/cartpole/est_reward2', Scalar(value=rew2.numpy()), state.stamp)
            self.logger.publish('/cartpole/action', Scalar(value=act.numpy()), state.stamp)
            self.critic.train()

        noise = 0.0

        if train:
            noise = torch.normal(0.0, sigma, size=act.shape)
    ...
    def learn(self, total):
        ...
        states, next_states, actions, rewards, dones = self.memory.sample(self.config.batch_size)

        with torch.no_grad():
            next_actions = self.target_actor(next_states)
            action_noise = torch.normal(0, 0.01, size=next_actions.shape)
